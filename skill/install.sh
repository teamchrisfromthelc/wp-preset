#!/usr/bin/env bash
# install.sh — install the wp-blueprint scaffolding command for your AI agent.
#
...
ROOT=$(cd "$HERE/.." && pwd)
SRC="$HERE/wp-blueprint/SKILL.md"
...
case "$TARGET" in
	claude)
		...
		write "$HOME/.claude/skills/wp-blueprint/SKILL.md" \
			"$(sed "s|\$WP_AI_SCAFFOLD|$ROOT|g" "$SRC")"
		echo "use: /wp-blueprint set up the blueprint for a plugin"
		;;
	codex)
		# Codex reads the file as a plain prompt body; frontmatter would show up
		# as literal text, so drop it.
		write "$HOME/.codex/prompts/wp-blueprint.md" "$(body)"
		echo "use: /wp-blueprint"
		;;
	cursor)
		...
		write "$HOME/.cursor/rules/wp-blueprint.mdc" \
			"$(printf -- '---\ndescription: %s\nalwaysApply: false\n---\n' "$(fm description)"; body)"
		echo "use: ask Cursor to set up the WordPress blueprint"
		;;
	print)
		body
		;;
